...
- Diagnosed Unity duplicate/replacement launcher icons at the BAMF object level: Unity/BAMF created one `Starting=true` Host application object from the mirrored launcher, then created a second runtime application object when the real X11 window was not attached to the first.
- Confirmed that environment-only BAMF hints are not universal: Electron Visual Studio Code can lose launch identity from its process environment, while other applications retain it.
- Added generic BAMF PID registration to the host bridge. The Debian launcher starts each Host launch behind a stopped gate, returns the real PID, Xenial `host-run` calls `org.ayatana.bamf.control.RegisterApplicationForPid` for the mirrored `debian-*` desktop path, then continues the gate so it `exec`s the real application with the same PID. This prevents a first-window race and preserves normal child handling.
- Confirmed PID registration places the authoritative `_BAMF_DESKTOP_FILE=/host-xdg/applications/debian-*.desktop` identity on both Electron Visual Studio Code and privileged/detached GUFW windows.
- Confirmed copied Debian `StartupWMClass` can veto that otherwise-correct explicit Host identity in Xenial BAMF; real mismatches included `StartupWMClass=Code` vs live `WM_CLASS=code` and GIMP's `gimp-3.0` vs live `gimp`/`Gimp`.
- Changed the generic synchronizer so **all** generated `debian-*.desktop` mirrors remove `StartupWMClass=`. Debian's real launchers and Xenial-native launchers remain untouched.
- Kept the mirrored `BAMF_DESKTOP_FILE_HINT` on all generated Host `Exec=` lines as part of the authoritative mirrored identity path. The current tested mirror also forces `StartupNotify=false`; that setting alone was not sufficient to solve the duplicate-icon issue.
- Confirmed the final global combination — authoritative mirrored desktop identity, early PID registration, and removal of mirrored `StartupWMClass` — resolves the duplicate/replacement Unity launcher behavior across the tested Host application classes, including Visual Studio Code, GIMP, Firefox ESR, Synaptic, GUFW/Firewall Configuration, and ordinary GTK applications.
- No per-application WM-class table, Electron exception, launcher hack, or downstream BAMF source patch is part of the accepted solution.
